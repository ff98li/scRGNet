...
After we have found the lower dimensional feature matrix, scRGNet fits this feature matrix to a KNN model: for a user defined value of $k$ (default uses the best heuristic value from the given feature matrix), for each cell sample, this KNN model selects K cells nearest in Euclidean distance, and then for the $k$ selected nearest cells of the current cell sample, fit them in an isolation forest model to prune outlier cells from the graph:
